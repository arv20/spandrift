...
# ---------------------------------------------------------------------------
# Real captured smolagents fixture
# ---------------------------------------------------------------------------

FIXTURE_DIR = Path(__file__).parent / "fixtures"


def test_ingest_real_smolagents_fixture():
    """Ingest a trace captured from a real SmolagentsInstrumentor()-instrumented run.

    The fixture was generated by running a CodeAgent with a DemoLLM model
    through openinference-instrumentation-smolagents, then serialized to OTLP
    JSON. This test ensures the ingest pipeline handles real-world attribute
    shapes (openinference.span.kind, llm.token_count.*, input.value, etc.)
    rather than only synthetic constructs.
    """
    fixture = FIXTURE_DIR / "smolagents_trace.json"
    assert fixture.exists(), f"Fixture not found: {fixture}"

    spans = load_otlp_json(fixture)

    # The fixture contains 3 spans from a CodeAgent.run() call:
    ...
    # Agent span — the top-level CodeAgent.run
    agent = by_kind[SpanKind.AGENT]
    assert "CodeAgent" in agent.name
    assert agent.input_tokens == 150
    assert agent.output_tokens == 30
    # input_value should be populated from the real input.value attribute
    assert agent.input_value is not None
    assert "What is 2 + 2?" in agent.input_value
    # input_hash should also be computed
    assert agent.input_hash is not None

    # Tool span — the FinalAnswerTool
    tool = by_kind[SpanKind.TOOL]
    assert "final_answer" in tool.name.lower() or "FinalAnswer" in tool.name
    assert tool.input_value is not None

    # Chain span — Step 1
    chain = by_kind.get(SpanKind.CHAIN)
    assert chain is not None
    assert "Step" in chain.name

    # Parent/child relationships should be intact
    assert tool.parent_span_id is not None
    assert chain.parent_span_id is not None
    # Both should be children of the agent span
    assert tool.parent_span_id == agent.span_id or chain.parent_span_id == agent.span_id
